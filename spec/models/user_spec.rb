...
RSpec.describe User, type: :model do
  ...
  describe 'ユーザー新規登録' do
    ...
    it "メールアドレスが重複しないこと" do
      @user.save
      another_user = FactoryBot.buld(:user)
    ...
    it "パスワードは、6文字以上であれば登録できること" do
      @user.password = "123456"
      @user.password_confirmation = "123456"
      expect(@user).to be_valid
    end
    it "パスワードは、5文字以下であれば登録できないこと"
      @user.password = "123456"
      @user.password_confirmation = "12345"
      @user.valid?
      expect(@user.errors.full_messages).to include("Password is too short (minimum is 6 characters)")
    it "パスワードは、半角英数字の混合が入力が必要である" do
    ...
    it "パスワードが存在してもパスワード（確認用）が空では登録できない" do
      ...
      @user.valid?
      expect(@user.errors.full_messages).to include("Password_confirmation is can't be blank")
    end
    ...
    it "ユーザー本名は、名前が空だと登録できない" do
      @user.first_name = nil
      @user.valid?
      expect(@user.errors.full_messages).to include("First_name can't ba blank")
    end
    it "ユーザー本名の苗字が全角でないと登録できない" do
      @user.family_name = "アイウエオ"
      @user.valid?
      expect(@user.errors.full_messages).to include("Please enter in full-width characters")
    end
